    ...
    <section id="problem" className="section-padding bg-black relative overflow-hidden border-t border-white/5">
      ...
      <div className="container max-w-7xl mx-auto px-6 relative z-10">
        <div className="flex flex-col mb-24 items-center text-center space-y-6">
          <div className="inline-flex items-center gap-3 px-3 py-1 rounded-full bg-red-500/5 border border-red-500/10 text-[9px] font-black text-red-400 uppercase tracking-[0.4em]">
          ...
          <h2 className="text-4xl md:text-7xl font-black font-heading text-white tracking-tighter leading-[1.0] uppercase max-w-4xl">
            Your Recruiters Are Spending Hours <br /><span className="text-red-500/20">On Work That Doesn't</span> Generate Revenue.
          </h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {problems.map((problem, i) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="glass-card p-10 border-red-500/5 hover:border-red-500/20 transition-all group flex flex-col items-center text-center group"
            >
              <div className="w-12 h-12 rounded-xl bg-red-500/5 flex items-center justify-center text-red-500 mb-6 group-hover:scale-110 transition-transform">
                <problem.icon size={20} strokeWidth={1} />
              </div>
              <p className="text-sm font-black text-slate-400 group-hover:text-white transition-colors duration-500 uppercase tracking-widest leading-relaxed">
                {problem.text}
              </p>
            </motion.div>
          ))}
